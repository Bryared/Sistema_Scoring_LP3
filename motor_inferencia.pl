...
%% ==========================================
%% MÓDULO 1: PREVENCIÓN DE FRAUDE (SEGURIDAD - 4 FILTROS)
%% ==========================================
...
%% Regla 1.2: Filtro Antifraude por Ubicación IP (Geolocalización).
es_fraude(Id) :- 
...
%% Regla 1.3: Filtro Antifraude por Fuerza Bruta.
es_fraude(Id) :- 
...
%% Regla 1.4: Filtro Anti-Bots (Velocidad de Llenado).
es_fraude(Id) :-
...
%% Regla 1.5: Suplantación IP vs Residencia (Filtro VPN).
es_fraude(Id) :-
    ubicacion_ip(Id, IP),
...
%% Regla 1.6: Lista Negra de Dispositivos (IMEI).
es_fraude(Id) :-
...
%% ==========================================
%% MÓDULO 4: PREVENCIÓN DE LAVADO DE ACTIVOS (AML DIFUSO)
%% ==========================================

%% Regla AML: Detectar triangulación financiera considerando comisiones de mula.
%% Condición: A -> B -> C -> A. Con montos considerables y margen de tolerancia.
%% Ejemplo: Margen de Tolerancia de 0.1 (10%).

alerta_aml(Id, ToleranciaDecimal, 'LAVADO DE ACTIVOS (TRIANGULACION CON COMISIONES)') :-
    transferencia(Id, B, Monto1, _), Monto1 > 10000,
    transferencia(B, C, Monto2, _),
    transferencia(C, Id, Monto3, _),
    Id \= B, B \= C, C \= Id,
    
    %% Aplicar Logica Difusa: El dinero que regresa no es el 100% por el costo de testaferros.
    RetornoMinimo is Monto1 * (1.0 - ToleranciaDecimal),
    Monto3 >= RetornoMinimo,
    Monto3 =< Monto1,
    !,
    format('~n[XAI] ALERTA AML: Triangulacion detectada (Smurfing/U-Turn Laundering).~n'),
...
%% Predicado auxiliar para extraer los nodos de la traza para el Grafo Visual
...
%% Regla SBS: Insolvencia Patrimonial (Liquidez vs Inmobiliario)
